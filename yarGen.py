...
def create_custom_rule(rule_name, strings, patterns=None, condition=None, meta=None, output_file=None):
    ...
    :param patterns: List of hex patterns to include
    :param condition: Custom condition (if None, will use 'any of them')
    :param meta: Dictionary with meta information (author, description, etc.)
    :param output_file: Output file (if None, will use 'custom_rule.yar')
    :return: Rule content as string
    """
    print("[+] Creating custom rule: %s" % rule_name)
    ...
    for i, string in enumerate(strings):
        ...
        rule += "      $s%d = \"%s\" %s\n" % (i, string, "fullword ascii")
    
    # Add hex patterns if provided
    if patterns:
...
def interactive_custom_rule():
    """
    Interactive function to create a custom YARA rule
    :return: None
    """
    print("\n[+] Interactive YARA Rule Creator")
    print("-----------------------------------\n")
    ...
    author = input("Author: ")
    reference = input("Reference (e.g., Threat Intel report link): ")
    
    meta = {
        "description": description if description else "Custom rule created by user input",
        "author": author if author else "yarGen Custom Rule Generator",
        "reference": reference if reference else "N/A",
        "date": get_timestamp_basic()
    }
    
    # Output file
    output_file = input("\nOutput file (leave empty for 'custom_rule.yar'): ")
    if not output_file:
        output_file = "custom_rule.yar"
...
if __name__ == '__main__':
    ...
    group_creation.add_argument('--excludegood', help='Force the exclude all goodware strings', action='store_true',
                                default=False)
    group_creation.add_argument('--custom-rule', help='Create a custom rule interactively without samples', action='store_true',
                                default=False)
    ...
    if not args.update and not args.m and not args.g and not args.custom_rule:
        parser.print_help()
        print("")
        print("""
